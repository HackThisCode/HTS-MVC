...
class tmvc
{
  ...
  public function main()
  {
    ...
    /* if path_info is empty, and the REQUEST_URI does not contain 'index.php'
     * the script is probably being run using URL redirection, and using the
     * whole REQUEST_URI in place of PATH_INFO would be appropriate.
     */
    if(empty($this->path_info) &&
       strpos($_SERVER['REQUEST_URI'], 'index.php') === FALSE &&
       !empty($_SERVER['REQUEST_URI'])) {
         $this->path_info = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
    }
